// Field-guide-style SVG placeholders for each species.
// Generated as data URIs so they always load.
// Each placeholder uses the species' kind + trophic for a thematic illustration:
// - colored wash background (paper-tone)
// - simple silhouette/glyph
// - latin name and a "specimen №" label
// This keeps the journal aesthetic and is 100% reliable.
...
function makePlaceholder(species, idx) {
  const tint = TROPHIC_TINT[species.trophic] || "#d4c4a0";
  const glyph = KIND_GLYPH[species.kind] || KIND_GLYPH.invertebrate;
  const num = String(idx + 1).padStart(3, "0");
  // Initial letters of name for monogram
  const monogram = species.name.split(" ").map(w => w[0]).join("").slice(0, 3).toUpperCase();
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" width="200" height="200">
    <defs>
      <filter id="grain"><feTurbulence type="fractalNoise" baseFrequency="0.9" numOctaves="2" seed="${idx}"/><feColorMatrix values="0 0 0 0 0.16 0 0 0 0 0.14 0 0 0 0 0.12 0 0 0 0.1 0"/></filter>
    </defs>
